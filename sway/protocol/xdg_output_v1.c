...
static void output_update(struct sway_xdg_output_v1 *xdg_output) {
...
static void output_destroy(struct sway_xdg_output_v1 *output) {
...
static void output_manager_handle_get_xdg_output(struct wl_client *client,
	...
	struct sway_xdg_output_manager_v1 *manager =
	...
	struct sway_xdg_output_v1 *_xdg_output, *xdg_output = NULL;
...
static void output_manager_bind(struct wl_client *wl_client, void *data,
	...
	struct sway_xdg_output_manager_v1 *manager = data;
...
static void handle_output_destroy(struct wl_listener *listener, void *data) {
	struct sway_xdg_output_v1 *output = wl_container_of(listener, output, destroy);
	output_destroy(output);
}

static void handle_output_description(struct wl_listener *listener,
		void *data) {
	struct sway_xdg_output_v1 *xdg_output =
...
static void add_output(struct sway_xdg_output_manager_v1 *manager,
		struct wlr_output_layout_output *layout_output) {
	struct sway_xdg_output_v1 *output = calloc(1, sizeof(*output));
	if (output == NULL) {
		return;
	}
...
static void output_manager_send_details(
		struct sway_xdg_output_manager_v1 *manager) {
	struct sway_xdg_output_v1 *output;
...
static void manager_destroy(struct sway_xdg_output_manager_v1 *manager) {
	struct sway_xdg_output_v1 *output, *tmp;
...
struct sway_xdg_output_manager_v1 *sway_xdg_output_manager_v1_create(
		struct wl_display *display, struct wlr_output_layout *layout) {
	struct sway_xdg_output_manager_v1 *manager = calloc(1, sizeof(*manager));
	if (manager == NULL) {
		return NULL;
	}
