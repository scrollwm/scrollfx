...
static void source_render(struct scene_node_source *source) {
	struct sway_scene_output *scene_output = source->scene_output;
	...
	sway_scene_output_set_position(scene_output, extents.x, extents.y);

	struct wlr_output_state state;
	wlr_output_state_init(&state);
	wlr_output_state_set_enabled(&state, true);
	wlr_output_state_set_custom_mode(&state, extents.width, extents.height, 0);
	bool ok = sway_scene_output_build_state(scene_output, &state, NULL) &&
	...
	sway_scene_output_send_frame_done(scene_output, &now);
}
...
static bool output_commit(struct wlr_output *output, const struct wlr_output_state *state) {
	...
	if (!(state->committed & WLR_OUTPUT_STATE_BUFFER)) {
		sway_log(SWAY_DEBUG, "Failed to commit capture output: missing buffer");
		return false;
	}
...
static void source_destroy(struct scene_node_source *source) {
	...
	sway_scene_output_destroy(source->scene_output);
...
static void source_handle_output_frame(struct wl_listener *listener, void *data) {
	...
	if (!sway_scene_output_needs_frame(source->scene_output)) {
		return;
	}
...
struct wlr_ext_image_capture_source_v1 *sway_ext_image_capture_source_v1_create_with_scene_node(
	...
	source->scene_output = sway_scene_output_create(scene, &source->output);
