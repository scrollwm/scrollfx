 ...
 */
struct sway_scene_subsurface_tree {
	...
	struct sway_scene_subsurface_tree *parent; // NULL for the top-level surface
...
static void subsurface_tree_addon_destroy(struct wlr_addon *addon) {
	struct sway_scene_subsurface_tree *subsurface_tree =
...
static struct sway_scene_subsurface_tree *subsurface_tree_from_subsurface(
		struct sway_scene_subsurface_tree *parent,
...
static bool subsurface_tree_reconfigure_clip(
	...
	if (wlr_box_empty(&subsurface_tree->clip)) {
		...
		sway_scene_node_set_enabled(&subsurface_tree->scene_surface->buffer->node, true);
		sway_scene_node_set_position(&subsurface_tree->scene_surface->buffer->node, 0, 0);
		...
		sway_scene_node_set_enabled(&subsurface_tree->scene_surface->buffer->node, intersects);

		if (intersects) {
			sway_scene_node_set_position(&subsurface_tree->scene_surface->buffer->node, clip.x, clip.y);
...
static void subsurface_tree_reconfigure(
	...
	struct sway_scene_node *prev = NULL;
	struct wlr_subsurface *subsurface;
	wl_list_for_each(subsurface, &surface->current.subsurfaces_below,
			current.link) {
		struct sway_scene_subsurface_tree *child =
			subsurface_tree_from_subsurface(subsurface_tree, subsurface);
		if (prev != NULL) {
			sway_scene_node_place_above(&child->tree->node, prev);
		}
		prev = &child->tree->node;

		sway_scene_node_set_position(&child->tree->node,
	...
	if (prev != NULL) {
		sway_scene_node_place_above(&subsurface_tree->scene_surface->buffer->node, prev);
	}
...
static void subsurface_tree_handle_surface_destroy(struct wl_listener *listener,
	...
	sway_scene_node_destroy(&subsurface_tree->tree->node);
}
...
static void subsurface_tree_handle_surface_map(struct wl_listener *listener,
	...
	sway_scene_node_set_enabled(&subsurface_tree->tree->node, true);
}
...
static void subsurface_tree_handle_surface_unmap(struct wl_listener *listener,
	...
	sway_scene_node_set_enabled(&subsurface_tree->tree->node, false);
}
...
static const struct wlr_addon_interface subsurface_tree_surface_addon_impl = {
	.name = "sway_scene_subsurface_tree",
	.destroy = subsurface_tree_surface_addon_destroy,
};

static struct sway_scene_subsurface_tree *scene_surface_tree_create(
	struct sway_scene_tree *parent, struct wlr_surface *surface);

static bool subsurface_tree_create_subsurface(
		struct sway_scene_subsurface_tree *parent,
		struct wlr_subsurface *subsurface) {
	struct sway_scene_subsurface_tree *child = scene_surface_tree_create(
...
static struct sway_scene_subsurface_tree *scene_surface_tree_create(
	...
	subsurface_tree->tree = sway_scene_tree_create(parent);
	if (subsurface_tree->tree == NULL) {
		goto error_surface_tree;
	}

	subsurface_tree->scene_surface =
		sway_scene_surface_create(subsurface_tree->tree, surface);
	...
	sway_scene_node_set_enabled(&subsurface_tree->tree->node, surface->mapped);
...
static struct sway_scene_subsurface_tree *get_subsurface_tree_from_node(
	...
	struct sway_scene_subsurface_tree *tree =
		wl_container_of(addon, tree, scene_addon);
	return tree;
}

static bool subsurface_tree_set_clip(struct sway_scene_node *node,
	...
	struct sway_scene_subsurface_tree *tree = get_subsurface_tree_from_node(node);
	if (tree) {
		if (tree->parent == NULL) {
